package com.asset.obs.common.logging;
...
public class AuditionLogger {
    ...
    private String createStandardProblemDetailMessage(final ProblemDetail standardProblemDetail) {
        // Formatted problemDetail message
        return String.valueOf(standardProblemDetail.getStatus()).concat(":").concat(standardProblemDetail.getTitle())
            .concat(":").concat(standardProblemDetail.getDetail());
    }

    private String createBasicErrorResponseMessage(final Integer errorCode, final String message) {
        // Formatted basic error response message
        return String.valueOf(errorCode).concat(":").concat(message);
    }
}